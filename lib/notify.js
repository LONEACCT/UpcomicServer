...
// Posts ONE combined notification for a batch of chapters that just went up
// together (bulk ZIP upload), instead of one Telegram/Push message per
// chapter — that flooding was the whole reason this exists.
async function notifyBulkChapters(comic, chapterNumbers) {
  const nums = chapterNumbers.slice().sort((a, b) => a - b);
  const label = nums.length > 1 ? `${nums[0]}–${nums[nums.length - 1]}` : String(nums[0]);
  // A representative "chapter" object so the existing single-chapter
  // notify functions can be reused as-is for the Telegram/push copy —
  // it links to the latest chapter in the batch.
  const latestChapter = db
    .prepare('SELECT * FROM chapters WHERE comic_id = ? AND chapter_number = ?')
    .get(comic.id, nums[nums.length - 1]);
  if (!latestChapter) return;

  const pseudoChapter = { ...latestChapter, chapter_number: label };
  await Promise.all([
    postToTelegram(comic, pseudoChapter),
    sendWebPush(comic, pseudoChapter),
    sendEmail(comic, pseudoChapter),
  ]);
}
...
module.exports = { notifyNewChapter, notifyBulkChapters, postToTelegram, sendWebPush, sendEmail, sendTestTelegramMessage };
